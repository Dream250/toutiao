...
public interface MessageDAO {
    ...
    @Select({"select ", INSERT_FIELDS, " from ", TABLE_NAME, " where to_id=#{userId}  and status <> 1 and  from_id=#{systemId}  order by id desc limit #{offset},#{limit}"})
    ...
    @Select({"select count(id) from ", TABLE_NAME, " where status <> 1 and has_read = 0 and to_id=#{userId}  and from_id = #{systemId}"})
    int getConversationSystemLetterUnReadCount(@Param("userId") int userId,
                                               @Param("systemId") int systemId);
